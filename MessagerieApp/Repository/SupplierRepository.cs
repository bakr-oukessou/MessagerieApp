	...
	public class SupplierRepository : ISupplierRepository
	{
		...
		public async Task<IEnumerable<Supplier>> GetAllSuppliersAsync()
		{
			...
			using (var connection = new SqlConnection(_connectionString))
			{
				...
				var command = new SqlCommand("SELECT * FROM Fournisseurs", connection);
		...
		public async Task<Supplier> GetSupplierByIdAsync(int id)
		{
			...
			{
				await connection.OpenAsync();
				var command = new SqlCommand("SELECT * FROM Fournisseurs WHERE Id = @Id", connection);
		...
		public async Task AddSupplierAsync(Supplier supplier)
		{
			...
			var command = new SqlCommand(
				@"INSERT INTO Fournisseurs (CompanyName, IsBlacklisted, BlacklistReason) 
		...
		public async Task DeleteSupplierAsync(int id)
		{
			...
			{
				await connection.OpenAsync();
				var command = new SqlCommand("DELETE FROM Fournisseurs WHERE Id = @Id", connection);
				command.Parameters.AddWithValue("@Id", id);
		...
		public async Task BlacklistSupplierAsync(int id, string reason)
		{
			using (var connection = new SqlConnection(_connectionString))
			{
				await connection.OpenAsync();
				var command = new SqlCommand(
					"UPDATE Fournisseurs SET IsBlacklisted = 1, BlacklistReason = @BlacklistReason WHERE Id = @Id",
					connection);
		...
		public async Task RemoveFromBlacklistAsync(int id)
		{
			using (var connection = new SqlConnection(_connectionString))
			{
				await connection.OpenAsync();
				var command = new SqlCommand(
					"UPDATE Fournisseurs SET IsBlacklisted = 0, BlacklistReason = NULL WHERE Id = @Id",
					connection);
